...
static void wrapfs_put_super(struct super_block *sb)
{
	...
	if(wrapfs_get_debug(sb) & DEBUG_SUPER)
		DEBUG_MESG("Enter");
...
static int wrapfs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	if(wrapfs_get_debug(dentry->d_sb) & DEBUG_SUPER)
		DEBUG_MESG("Enter");
...
static int wrapfs_remount_fs(struct super_block *sb, int *flags, char *options)
{
	...
	if(options!=NULL) {
		err = wrapfs_parse_options(options, &mount_options);
		if(err<0)
		{
	...
	if ((*flags & ~(MS_RDONLY | MS_MANDLOCK | MS_SILENT)) != 0) {
		printk(KERN_ERR "wrapfs: remount flags 0x%x unsupported\n", *flags);
		err = -EINVAL;
		goto out;
	}
	...
	wrapfs_set_mount_options(sb, mount_options);
out:
	return err;
}
...
static void wrapfs_evict_inode(struct inode *inode)
{
	...
	if(wrapfs_get_debug(inode->i_sb) & DEBUG_SUPER)
		DEBUG_MESG("Enter");
